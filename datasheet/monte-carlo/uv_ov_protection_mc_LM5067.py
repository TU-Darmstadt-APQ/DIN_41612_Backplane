...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Datasheet parameter calculator for the LM5067 input protection ic using Monte Carlo methods."
    )
    parser.add_argument(
        "-v",
        "--version",
...
def plot_result(
    ...
    # Min/Max is swapped, because we treat min/max as absolute values
    print(f"Min:{upper_bound:.3f}, Typical: {max_res:.3f} Max: {lower_bound:.3f}")
...
print("Do note: All values are treated as absolute values for the purpose of min/max.")

# These random values can be reused for as long as they are used for independent and uncorrelated variables
...
v_sense = np.random.normal(loc=-2.5, scale=0.05 / 3, size=N_SAMPLES)  # 2.5 V +- 0.05 V

print("Calculating undervoltage thresholds")
r1, r2 = 45.3e3 * tol_1, 17.4e3 * tol_2  # Undervoltage
...
# uv hysteresis. A 22 µA current source connected to the negative Vee pulls uv low
vout_hysteresis = (r1 * r2) / (r1 + r2) * np.random.normal(loc=22e-6, scale=(12e-6) / 3, size=N_SAMPLES)
result = res_divider(vout=v_sense - vout_hysteresis, res1=r1, res2=r2)
plot_result(result, confidence_interval=N_SIGMA, is_silent=args.silent)
plt.clf()

print("Calculating overvoltage thresholds")
r1, r2 = 45.3e3 * tol_1, 4.7e3 * tol_2  # Overvoltage
result = res_divider(vout=v_sense + 0, res1=r1, res2=r2)
plot_result(result, confidence_interval=N_SIGMA, is_silent=args.silent)
plt.clf()

print("Calculating overvoltage hysteresis")
# ov hysteresis. A 22 µA current source connected to ground pulls ov high
result = res_divider(vout=v_sense + vout_hysteresis, res1=r1, res2=r2)
plot_result(result, confidence_interval=N_SIGMA, is_silent=args.silent)
plt.clf()

print("Calculating overcurrent thresholds")
v_sense = np.random.normal(loc=-50 / 1000, scale=6 / 1000 / 3, size=N_SAMPLES)  # 50 mV +- 6 mV
r1 = 3e-3 * tol_1
result = v_sense / (r1)
plot_result(result, confidence_interval=N_SIGMA, is_silent=args.silent)
